...
function ObjectiveCard({ objective }: { objective: Objective }) {
  ...
  return (
    ...
    <Pressable className="rounded-2xl p-5 bg-goalmax-surface border border-goalmax-border mb-4 active:opacity-90">
      {/* Header */}
      <View className="flex-row items-center mb-4">
        ...
        <View className="ml-4 flex-1">
          ...
          <Text className="text-goalmax-text text-lg font-bold">{objective.name}</Text>
        </View>
        {objective.isPaused && (
          <View className="bg-goalmax-muted/20 px-2 py-1 rounded">
            <Text className="text-goalmax-muted text-xs">PAUSED</Text>
          </View>
        )}
      </View>
      ...
      <View className="mb-4">
        <View className="flex-row items-center justify-between mb-2">
          <Text className="text-goalmax-text-secondary text-sm">Overall Progress</Text>
          <Text className="text-goalmax-accent font-semibold">{Math.round(overallProgress)}%</Text>
        </View>
      ...
      <View className="flex-row">
        <View className="flex-1">
          <Text className="text-goalmax-text font-semibold">{objective.pillars.length}</Text>
          <Text className="text-goalmax-text-secondary text-sm">Pillars</Text>
        </View>
        <View className="flex-1">
          <Text className="text-goalmax-text font-semibold">{objective.metrics.length}</Text>
          <Text className="text-goalmax-text-secondary text-sm">Metrics</Text>
        </View>
        <View className="flex-1">
          <Text className="text-goalmax-text font-semibold">
            {completedToday}/{activeRituals}
          </Text>
          <Text className="text-goalmax-text-secondary text-sm">Rituals Today</Text>
        </View>
      </View>

      {/* Target Outcome */}
      <View className="mt-4 pt-4 border-t border-goalmax-border">
        <Text className="text-goalmax-text-secondary text-xs mb-1">TARGET</Text>
        <Text className="text-goalmax-text text-sm" numberOfLines={2}>
    ...
    <View className="items-center py-16 px-8">
      ...
      <Link href="/create-objective" asChild>
        <Pressable className="bg-goalmax-accent rounded-xl py-4 px-8 active:opacity-80">
          <Text className="text-goalmax-bg font-semibold text-lg">Create Your First Objective</Text>
        </Pressable>
      </Link>
    </View>
  );
    ...
    <SafeAreaView className="flex-1 bg-goalmax-bg" edges={['bottom']}>
      <ScrollView className="flex-1 px-5 pt-4">
            ...
            <View className="flex-row items-center justify-between mb-4">
              ...
              <Link href="/create-objective" asChild>
                <Pressable className="flex-row items-center">
                  ...
                  <Text className="text-goalmax-accent ml-2 font-medium">Add</Text>
                </Pressable>
              </Link>
            </View>
        ...
        {pausedObjectives.length > 0 && (
          <>
            <Text className="text-goalmax-text-secondary text-sm tracking-wide mt-6 mb-4">
        ...
        {objectives.length > 0 && activeObjectives.length === 0 && (
          <View className="items-center py-8">
            <Link href="/create-objective" asChild>
              <Pressable className="bg-goalmax-accent rounded-xl py-4 px-8 active:opacity-80">
                <Text className="text-goalmax-bg font-semibold">Add New Objective</Text>
              </Pressable>
            </Link>
          </View>
        )}
